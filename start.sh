...
if [[  -n "${LEAN_REDIS_PASSWORD_FILE}" ]]; then
  LEAN_DB_PASSWORD=$(cat "${LEAN_REDIS_PASSWORD_FILE}")
  export LEAN_REDIS_PASSWORD
fi

if [[  -n "${LEAN_DB_HOST_FILE}" ]]; then
  LEAN_DB_PASSWORD=$(cat "${LEAN_DB_HOST_FILE}")
  export LEAN_DB_HOST
fi

if [[  -n "${LEAN_DB_DATABASE_FILE}" ]]; then
  LEAN_DB_PASSWORD=$(cat "${LEAN_DB_DATABASE_FILE}")
  export LEAN_DB_DATABASE
fi

if [[  -n "${LEAN_DB_USER_FILE}" ]]; then
  LEAN_DB_PASSWORD=$(cat "${LEAN_DB_USER_FILE}")
  export LEAN_DB_USER
fi

if [[  -n "${LEAN_EMAIL_SMTP_USERNAME_FILE}" ]]; then
  LEAN_DB_PASSWORD=$(cat "${LEAN_EMAIL_SMTP_USERNAME_FILE}")
